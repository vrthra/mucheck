...
isAVL :: AVLTree k v -> Bool
isAVL Leaf = True
isAVL (AVLTree k v _ _ l r) = and [isAVL l, isAVL r, (abs  (height r - height l)) <= 1]



prop_bst :: (Ord a, Num a) => [(a,a)] -> Bool
prop_bst xs = (L.sort  (map fst xs)) == map fst (toList $ fromList xs)


-- | prop_delta :: (Ord a, Num a) => [a] -> Bool
-- | prop_delta = checkDelta . fromList. L.nub

prop_avl :: (Ord a, Num a) => [(a,a)] -> Bool
